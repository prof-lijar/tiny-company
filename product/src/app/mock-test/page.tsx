    ...
    <div className={`min-h-screen ${isExamMode ? 'bg-slate-50' : 'bg-white'} py-12 px-4`}>
              ...
              <div key={t.id} className="p-8 bg-white border border-slate-200 rounded-2xl shadow-sm text-left space-y-6">
                <div className="space-y-2">
                  ...
                  <div className="flex gap-4 text-sm text-slate-500">
                    <span className="flex items-center gap-1"><Timer size={14} /> {t.sections.reduce((acc, s) => acc + s.durationMinutes, 0)} mins</span>
                    <span className="flex items-center gap-1"><AlertCircle size={14} /> {t.sections.reduce((acc, s) => acc + s.questions.length, 0)} Questions</span>
                  </div>
                </div>
      ...
      {isTestActive && (
        <div className={`max-w-7xl mx-auto space-y-6 ${isExamMode ? 'grid grid-cols-12 gap-6' : 'block'}`}>
          {/* Header */}
          <div className={`flex flex-col md:flex-row justify-between items-center bg-white p-4 rounded-2xl border border-slate-200 shadow-sm sticky top-4 z-10 gap-4 ${isExamMode ? 'col-span-12' : ''}`}>
            ...
            <div className="flex items-center gap-2 overflow-x-auto w-full md:w-auto">
              {!isExamMode && test.sections.map((sec, idx) => (
                <button
                  key={sec.id}
              ...
              <div className="bg-white p-6 rounded-3xl border border-slate-200 shadow-sm sticky top-24">
                ...
                <div className="mt-8 p-4 bg-slate-50 rounded-2xl border border-slate-100 text-xs text-slate-500 leading-relaxed">
                  <div className="font-bold text-slate-700 mb-1 flex items-center gap-1">
                    <AlertCircle size={12} /> Exam Rules
                  </div>
