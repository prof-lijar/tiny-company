...
const MOCK_USER_MISTAKES: UserMistake[] = [
  { questionId: 'r1', category: 'reading', tags: ['Grammar: -기에'], timestamp: Date.now() - 86400000 },
  { questionId: 'r2', category: 'reading', tags: ['Grammar: -기에'], timestamp: Date.now() - 86400000 * 2 },
  { questionId: 'r3', category: 'reading', tags: ['Vocab: Environment'], timestamp: Date.now() - 86400000 * 3 },
  { questionId: 'l1', category: 'listening', tags: ['Grammar: -기에'], timestamp: Date.now() - 86400000 * 4 },
  ...
  try {
    ...
    const topWeaknesses = sortedTags.map(([tag, count]) => {
      ...
      if (tag.startsWith('Grammar:')) {
        ...
        recommendation = `Review the grammar lesson for ${tag.split(': ')[1]} in the Grammar Library. Focus on its usage in complex sentences.`;
