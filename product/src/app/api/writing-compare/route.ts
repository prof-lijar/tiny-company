...
import { getModelEssays } from '@/lib/model-essays';
...
export async function POST(req: Request) {
  try {
    ...
    // We use id as a fallback for promptId since the ModelEssay type doesn't explicitly have promptId
    const modelEssay = essays.find(m => m.id === promptId);
    ...
    keyConcepts.forEach(concept => {
      if (!userEssay.includes(concept.term) && modelEssay.content.includes(concept.term)) {
