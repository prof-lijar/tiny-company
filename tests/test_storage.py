...
class TestTraceStorage(unittest.TestCase):
    ...
    def test_save_and_get_trace(self):
        ...
        self.storage.save_trace(trace_id, "agent-1", "session-1", metadata)
        
        trace = self.storage.get_trace_metadata(trace_id)
    ...
    def test_append_and_get_logs(self):
        trace_id = "trace-123"
        self.storage.save_trace(trace_id, "agent-1", "session-1", {})
        ...
        logs = self.storage.get_trace_logs(trace_id)
    ...
    def test_recent_traces(self):
        self.storage.save_trace("t1", "a1", "s1", {})
        self.storage.save_trace("t2", "a2", "s2", {})
        
        recent = self.storage.get_recent_traces(limit=1)
