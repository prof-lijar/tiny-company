...
def parse(
    ...
    suggest_fix: bool = typer.Option(False, "--suggest-fix", help="Suggest fixes for detected reasoning failures")
) -> None:
    """
        ...
        if output:
            with open(output, 'w') as f:
                ...
                for seg in report.narrative:
                    ...
                    if seg.suggested_fix:
                        f.write(f"  - **Suggested Fix**: {seg.suggested_fix.suggested_modification}\n")
                        f.write(f"  - **Rationale**: {seg.suggested_fix.rationale}\n")
                        f.write(f"  - **Confidence**: {seg.suggested_fix.confidence_score}\n")
            ...
            for seg in report.narrative:
                ...
                if seg.suggested_fix:
                    typer.echo(f"  - [FIX-IT] Suggested Modification: {seg.suggested_fix.suggested_modification}")
                    typer.echo(f"  - [FIX-IT] Rationale: {seg.suggested_fix.rationale}")
                    typer.echo(f"  - [FIX-IT] Confidence: {seg.suggested_fix.confidence_score}")
